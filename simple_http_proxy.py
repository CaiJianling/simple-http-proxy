...
class SimpleHttpProxy(object):
    ...
    def __handle_socks5(self, socket_client, req_data):
        '''
        处理SOCKS5协议
        
        参数：socket_client 客户端套接字
        参数：req_data 初始请求数据
        '''
        debug('socks5', 'Handling SOCKS5 request')
        
        # SOCKS5握手阶段
        if len(req_data) >= 3 and req_data[0] == 0x05:
            # 检查认证方法
            nmethods = req_data[1]
            ...
            # 只支持无认证(0x00)
            if 0x00 in methods:
                socket_client.send(b'\x05\x00')  # 选择无认证
                debug('socks5', 'SOCKS5 handshake completed - no authentication')
            else:
                socket_client.send(b'\x05\xFF')  # 无可接受的方法
                socket_client.close()
                debug('socks5', 'SOCKS5 handshake failed - no acceptable authentication method')
                return
        ...
        try:
            ...
            if len(req_data) < 10:
                debug('socks5', 'Invalid SOCKS5 request - too short')
                socket_client.close()
                return
            
            debug('socks5', 'SOCKS5 connection request: %s' % req_data[:20].hex())
            ...
            if version != 0x05:
                debug('socks5', 'Invalid SOCKS5 version: %d' % version)
                socket_client.close()
                return
            
            if cmd != 0x01:  # 只支持CONNECT命令
                debug('socks5', 'Unsupported SOCKS5 command: %d' % cmd)
                socket_client.send(b'\x05\x07\x00\x01\x00\x00\x00\x00\x00\x00')  # 命令不支持
                socket_client.close()
                return
            ...
            if addr_type == 0x01:  # IPv4
                if len(req_data) < 10:
                    debug('socks5', 'Invalid IPv4 address')
                    socket_client.close()
                    return
                target_addr = socket.inet_ntoa(req_data[4:8])
                target_port = int.from_bytes(req_data[8:10], 'big')
                debug('socks5', 'IPv4 target: %s:%d' % (target_addr, target_port))
            elif addr_type == 0x03:  # 域名
                if len(req_data) < 5:
                    debug('socks5', 'Invalid domain name')
                    socket_client.close()
                    return
                domain_length = req_data[4]
                if len(req_data) < 5 + domain_length + 2:
                    debug('socks5', 'Invalid domain name length')
                    socket_client.close()
                    return
                target_addr = req_data[5:5+domain_length].decode()
                target_port = int.from_bytes(req_data[5+domain_length:7+domain_length], 'big')
                debug('socks5', 'Domain target: %s:%d' % (target_addr, target_port))
            elif addr_type == 0x04:  # IPv6
                debug('socks5', 'IPv6 not supported')
                socket_client.send(b'\x05\x08\x00\x01\x00\x00\x00\x00\x00\x00')  # 地址类型不支持
                socket_client.close()
                return
            else:
                debug('socks5', 'Invalid address type: %d' % addr_type)
                socket_client.send(b'\x05\x07\x00\x01\x00\x00\x00\x00\x00\x00')  # 地址类型不支持
                socket_client.close()
                return
            
            # 连接到目标服务器
            try:
                socket_server = self.__connect(target_addr, target_port)
                debug('socks5', 'Successfully connected to %s:%d' % (target_addr, target_port))
                
                # 发送成功响应
                response = b'\x05\x00\x00\x01\x00\x00\x00\x00\x00\x00'
                socket_client.send(response)
                debug('socks5', 'SOCKS5 connection established')
                
                # 开始转发数据
                self.__nonblocking(socket_client, socket_server)
                
            except Exception as e:
                debug('socks5', 'Failed to connect to target: %s' % str(e))
                socket_client.send(b'\x05\x05\x00\x01\x00\x00\x00\x00\x00\x00')  # 连接失败
                socket_client.close()
                
        except Exception as e:
            debug('socks5', 'SOCKS5 error: %s' % str(e))
            socket_client.close()
    ...
    def __connect(self, host, port):
        ...
        try:
            ...
            debug('connect', 'Connecting to %s:%s' % (host, port))
            ...
            tmp_socket.settimeout(30)  # 增加超时时间到30秒
            tmp_socket.connect(target_addr)
            debug('connect', 'Successfully connected to %s:%s' % (host, port))
            return tmp_socket
        except Exception as e:
            debug('error', 'Failed to connect to %s:%s - %s' % (host, port, str(e)))
            raise
         
    def __proxy(self, socket_client):
        '''
        代理核心程序 - 支持HTTP/HTTPS和SOCKS5协议
        ...
        # 调试：显示接收到的原始数据
        debug('raw_data', 'Received %d bytes: %s' % (len(req_data), req_data[:200]))

        # 检查是否为SOCKS5协议
        if req_data.startswith(b'\x05'):
            self.__handle_socks5(socket_client, req_data)
            return
        ...
        # 其他未知方法
            ...
            debug('error', 'Unknown HTTP method: %s' % http_packet.method)
            error_msg = b'HTTP/1.1 405 Method Not Allowed\r\nContent-Length: 0\r\n\r\n'
            socket_client.send(error_msg)
            socket_client.close()
            return
